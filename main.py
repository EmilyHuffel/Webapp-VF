    ...
    df_daily_reviews = pd.read_sql_query(
        f"""
            select 
                ...
                DATE(rev.datetime) as review_day,
                count(*) as n_reviews
            from 
                public.reviews rev
            ...
            group by review_day, res.colophon_data_city 
            """,  # noqa: F541
        con=engine,
    )
...
date_min, date_max = st.slider(
    ...
    value=(datetime.date(2022, 1, 1), datetime.date(2022, 12, 31)),
)
...
filtered_data = data.loc[((data["review_day"] >= date_min) & (data["review_day"] <= date_max))].sort_values(
    by="review_day"
)
print(filtered_data)
...
data_summed = filtered_data[["city", "n_reviews"]].groupby("city", as_index=False).sum()
